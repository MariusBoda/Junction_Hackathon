The DEMO folder contains five scripts:

    - filter.py : this is a helper script that does the preprocessing of the 2D svg floor maps. This script filters the files based on user selections done in the interface.

    - get_stroke_widths.py : this is a helper script that assists in filtering and preprocessing the data be getting the widths of the strokes used in the 2D floor maps.

    - multi_2d_to_3d.py : this is a helper script that take the different 2D floor maps (such as floor maps for floors 1-7) and creates a 3D model from them.

    - web_app.py : this is a streamlit application that launches a localhost web UI that can be used to run the demo. In the UI there is the option to upload a directory/folder containing all the 2D svg floor maps. After running in the background the UI will open up Blender with the generated 3D model. The script will also save the output file as output.blend. This file can be exported as a GLB and opened in the display_3d_model.py app.

    - display_3d_model.py : this is another streamlit application that launches a localhost web UI that can be used to view the 3D model in the browser. For this one it is required to upload the GLB file. The web_app.py creates a blend file called output.blend, this file can be exported and viewed in the browser.

DEMO VIDEOS:

https://drive.google.com/file/d/18aUhQWOFmRV8kD4NR1utztUxzUA4NuKR/view?usp=share_link

REQUIREMENTS:

Blender, various libraries, terminal
